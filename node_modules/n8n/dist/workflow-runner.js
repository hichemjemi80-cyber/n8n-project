...
let WorkflowRunner = class WorkflowRunner {
    ...
    async runMainProcess(executionId, data, loadStaticData, restartExecutionId) {
        ...
        let workflowTimeout = workflowSettings.executionTimeout ?? config_2.default.getEnv('executions.timeout');
        if (workflowTimeout > 0) {
            workflowTimeout = Math.min(workflowTimeout, config_2.default.getEnv('executions.maxTimeout'));
        }
        let pinData;
        ...
        try {
            ...
            if (workflowTimeout > 0) {
                const timeout = Math.min(workflowTimeout, config_2.default.getEnv('executions.maxTimeout')) * 1000;
    ...
    async enqueueExecution(executionId, data, loadStaticData, realtime) {
        ...
        const workflowExecution = new p_cancelable_1.default(async (resolve, reject, onCancel) => {
            ...
            const { queueRecoveryInterval } = typedi_1.Container.get(config_1.GlobalConfig).queue.bull;
