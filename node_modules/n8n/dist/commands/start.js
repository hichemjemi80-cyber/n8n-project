...
const local_task_manager_1 = require("../runners/task-managers/local-task-manager");
...
class Start extends base_command_1.BaseCommand {
    ...
    async init() {
        ...
        if (!config_1.default.getEnv('license.autoRenewEnabled') && this.instanceSettings.isLeader) {
        ...
        if (!this.globalConfig.taskRunners.disabled) {
            typedi_1.Container.set(task_manager_1.TaskManager, new local_task_manager_1.LocalTaskManager());
            const { TaskRunnerServer } = await Promise.resolve().then(() => __importStar(require('../runners/task-runner-server')));
            const taskRunnerServer = typedi_1.Container.get(TaskRunnerServer);
            await taskRunnerServer.start();
            const { TaskRunnerProcess } = await Promise.resolve().then(() => __importStar(require('../runners/task-runner-process')));
            const runnerProcess = typedi_1.Container.get(TaskRunnerProcess);
