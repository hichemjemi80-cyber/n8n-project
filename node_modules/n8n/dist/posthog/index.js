...
let PostHogClient = class PostHogClient {
    ...
    async init() {
        const enabled = config_2.default.getEnv('diagnostics.enabled');
        if (!enabled) {
            return;
        }
        const { PostHog } = await Promise.resolve().then(() => __importStar(require('posthog-node')));
        this.postHog = new PostHog(config_2.default.getEnv('diagnostics.config.posthog.apiKey'), {
            host: config_2.default.getEnv('diagnostics.config.posthog.apiHost'),
        });
