                ...
                if (resource === 'video') {
                    if (operation === 'getAll') {
                        ...
                        if (filters.publishedAfter) {
                            const publishedAfter = luxon_1.DateTime.fromISO(filters.publishedAfter);
                            if (publishedAfter.isValid) {
                                filters.publishedAfter = publishedAfter.setZone(this.getTimezone()).toISO();
                            }
                            else {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The value "${filters.publishedAfter}" is not a valid DateTime.`);
                            }
                        }
                        if (filters.publishedBefore) {
                            const publishedBefore = luxon_1.DateTime.fromISO(filters.publishedBefore);
                            if (publishedBefore.isValid) {
                                filters.publishedAfter = publishedBefore.setZone(this.getTimezone()).toISO();
                            }
                            else {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The value "${filters.publishedBefore}" is not a valid DateTime.`);
                            }
                        }
                        Object.assign(qs, options, filters);
